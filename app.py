...
def render_audio_player_b64(audio_b64: str, consent: bool | None = None):
    ...
    if consent:
        # Keep element present to avoid iOS quirks
        controls_attr = ""
    ...
    # Safe HTML block (only Python substitutions; no JS braces here)
    audio_html = f"""
    ...
    script_html = ""
    if consent:
        # IMPORTANT: use %s substitution so curly braces in JS remain literal
        script_html = """
        <script>
          (function() {
            var id = "%s";
            ...
            function createUnlock(a) {
              ...
              function unlock() {
                try { a.load(); } catch(_) { /* some browsers need load() before play */ }
                a.muted = false;
                a.volume = 1;
