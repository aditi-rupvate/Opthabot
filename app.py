...
def handle_query_logic(query: str, session_id: str = None):
    ...
    # Use a copy of the history so the memory is fresh for each call
...
# 1. Display the existing chat history
for i, entry in enumerate(st.session_state.chat_history):
    if "user" in entry: 
        ...
        if st.session_state.voice_enabled and "audio_b64" in entry:
            audio_html = f"""<audio controls autoplay><source src="data:audio/mp3;base64,{entry['audio_b64']}" type="audio/mp3"></audio>"""
...
# 2. If the last message was from the user, generate a response
if st.session_state.chat_history and "user" in st.session_state.chat_history[-1] and not st.session_state.processing:
    ...
    with st.spinner("Thinking..."):
        answer, pdf_filename = handle_query_logic(user_prompt)
...
# 3. Show input controls only when not processing a response
