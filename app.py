...
for message in st.session_state.chat_history:
    ...
    with st.chat_message(role):
        ...
        # Add a play button for the audio if it exists
...
# 2. Process a new prompt if one has been submitted
if st.session_state.processing_prompt:
    last_user_prompt = st.session_state.chat_history[-1]["user"]
    with st.chat_message("bot"):
        with st.spinner("Thinking..."):
            history_for_logic = st.session_state.chat_history[:-1]
            answer, pdf_filename = get_bot_response(last_user_prompt, history_for_logic, st.session_state.get("session_id"))
...
# 3. Display input widgets
if not st.session_state.processing_prompt:
    if st.session_state.voice_enabled:
